...
function callCortexAnalystAPI(string $prompt): array {
    ...
    // Get the login token (file, else ENV fallback)
    $token = '';
    if (file_exists($tokenFile)) {
        $token = trim(file_get_contents($tokenFile));
    } else {
        $token = getenv('CORTEX_TOKEN');
        if (empty($token)) {
            $token = getenv('SNOWFLAKE_OAUTH_TOKEN');
        }
        if (empty($token)) {
            return ['error' => 'Error: OAuth token not found. Set CORTEX_TOKEN environment variable or provide token_file.'];
        }
    }
